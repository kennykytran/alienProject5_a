...
class AlienFleet:
    ...
    alien_images = [[pg.transform.rotozoom(pg.image.load(f'images/alien_{m}{n}.png'), 0, 1) for n in range(2)] for m in
                    range(3)]
    ufo_imgs = [pg.transform.rotozoom(pg.image.load(f'images/alien2_{n}.png'), 0, 1) for n in range(2)]
    ...
    def create_ufo(self):
        ...
        alien = Alien(game=self.game, sound=self.sound, alien_index=3, ul=(x, y), v=self.v, image_list=images)
...
class UfoFleet:
    ...
    def update(self):
        if pg.time.get_ticks() % 400 == 1:
            if randint(0, 9) == 1 & self.length() == 0:
                self.create_fleet()
